...
class ComposerInstallerPlugin implements PluginInterface, EventSubscriberInterface
{
	...
	public function activate(Composer $composer, IOInterface $io)
	{
		...
		foreach([
			...
			if($matches = glob($glob)) {
				$this->$var = ceil(max(array_map(function($e) { return explode('-', pathinfo($e, PATHINFO_FILENAME), 2)[0]; }, $matches))/10)+1;
			}
		}
	...
	{
		if(!in_array($event->getOperation()->getPackage()->getType(), ['heroku-sys-php', 'heroku-sys-hhvm', 'heroku-sys-php-extension', 'heroku-sys-hhvm-extension'])) return;
