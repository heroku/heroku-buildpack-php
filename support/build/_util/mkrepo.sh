...
python -c 'import sys, json; from distutils import version; print json.dumps({"packages": [ sorted([json.load(open(item)) for item in sys.argv[1:]], key=lambda package: version.LooseVersion(package.get("require", {}).get("heroku-sys/hhvm", package.get("require", {}).get("heroku-sys/php", "0.0.0"))), reverse=True) ] })' $manifests

if [[ -t 1 ]]; then
	echo "-----> Done. Run 's3cmd --ssl --access_key=\$AWS_ACCESS_KEY_ID --secret_key=\$AWS_SECRET_ACCESS_KEY --acl-public put packages.json s3://${S3_BUCKET}${S3_PREFIX}packages.json' to upload repository." >&2
fi