...
# for f in support/build/packgaes/php-{5,7}.* support/build/packages/extensions/no-debug-non-zts-201*/{redis,blackfire,imagick}-*; do docker run --rm --tty --interactive --env-file=../dockerenv.heroku-22 heroku-php-builder-heroku-22 deploy.sh $f; done
args=()
for var in "$@"; do
	expanded="$(pwd)/$var"
	if [[ -f $expanded ]]; then
