...
- `S3_REGION`, to be set to the AWS region name (e.g. "`s3.eu-west-1`") for any non-standard region, otherwise it will default to "`s3.us-east-1`".
...
First, create, in a secure location on your file system, a `heroku-php-s3.dockerenv` file with the following contents:
    ...
    S3_REGION=… # only needed if you want to use a region other than "s3.us-east-1"
...
- All manifests generated by Bob formulas, by `mkrepo.sh` and by `sync.sh` use an S3 region of "s3.us-east-1" by default, so resulting URLs look like "`https://<your-bucket-name>.s3.us-east-1.amazonaws.com/your-prefix/...`". You can `heroku config:set S3_REGION` to change "s3.us-east-1" to another region such as "s3.eu-west-1".
