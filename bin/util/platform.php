...
if(!$have_runtime_req) {
	if($have_dev_runtime_req) {
		...
		// that's problematic, because requirements in there may effectively result in a rule like "8.0.*", but we'd next write "^7.0.0" into our "require" to have a sane default for all stacks, and that'd blow up in CI where dev dependencies are installed
